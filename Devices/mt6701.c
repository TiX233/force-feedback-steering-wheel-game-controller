#include "mt6701.h"

#define PI  3.14159265358979f

// 阻塞读取
void mt6701_read(struct mt6701_stu *mt){
    mt->read_reg(mt, 0x03, mt->data_buffer, 2);
}

// 非阻塞读取
void mt6701_read_dma(struct mt6701_stu *mt){
    mt->read_reg_dma(mt, 0x03, mt->data_buffer, 2);
}

// 转换读出来的值为角度
float mt6701_trans_angle(struct mt6701_stu *mt){
#ifdef MT6701_USESPI_16BIT
    mt->data_row = (mt->data_buffer[0] >> 2) + (mt->data_buffer[1] << 6);
#else
    mt->data_row = (mt->data_buffer[1] >> 2) + (mt->data_buffer[0] << 6);
#endif
    
    return mt->data_row*(360.0f/16384.0f);
}

// 转换读出来的值为弧度
float mt6701_trans_rad(struct mt6701_stu *mt){
#ifdef MT6701_USESPI_16BIT
    mt->data_row = (mt->data_buffer[0] >> 2) + (mt->data_buffer[1] << 6);
#else
    mt->data_row = (mt->data_buffer[1] >> 2) + (mt->data_buffer[0] << 6);
#endif
    float rad = mt->data_row*(2*PI/16384.0f) + mt->rad_offset;
    
    if(rad >= (2*PI)){
        rad -= (2*PI);
    }

    return rad;
}

// 设置弧度偏置
void mt6701_set_rad_offset(struct mt6701_stu *mt, float offset){

    while(offset < 0.0f){
        offset += (2*PI);
    }
    while(offset >= (2*PI)){
        offset -= (2*PI);
    }
    mt->rad_offset = offset;
}
